...
def DataGen(data_x,data_y,ClassNum,SampleNum,TestNum,TrainNum):
...
if __name__ == '__main__':
    ...
    matFn='../../TangutOCR/WH70_C36_S100.mat'
    data=sio.loadmat(matFn)
    data_x=data["data_x"]
    data_y=data["data_y"]
    x_test=np.transpose(data_x[:,321])
    y_test=np.transpose(data_y[:,321])-1
    print('object class:', y_test)

    CharImg = x_test.reshape(1, img_rows, img_cols,1)


    # dlg = win32ui.CreateFileDialog(1)
    # dlg.SetOFNInitialDir('E:\\T_Char\\CharSample_Gen')
    # dlg.DoModal()
    # filename = dlg.GetPathName()
    # print(filename)
    # CharImg=cv2.imread(filename)
    # cv2.imshow(CharImg)




    model = load_model('CNN_WH70_C36_S100.h5')
    score = model.predict(CharImg, verbose=0)
